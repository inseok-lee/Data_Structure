...
// Post: If a node is found with the same key as item?, then 
//       nodePtr points to that node and parentPtr points to its 
//       parent node. If the root node has the same key as item?,
...
bool TreeType::Ancestors_recursive(TreeNode* tree, ItemType value)
{
	bool found = false;
	...
	if (tree->info < value)
		found = Ancestors_recursive(tree->right, value);
	else
		found = Ancestors_recursive(tree->left, value);
...
int TreeType::Smaller(TreeType tree, int value) {
