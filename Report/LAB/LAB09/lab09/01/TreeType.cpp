...
TreeNode* TreeType::PtrToSuccessor_Recursive(TreeNode* &tree) {
	if (tree->left != NULL) {
		PtrToSuccessor_Recursive(tree->left);
	}
	else {
		std::cout << "\nroot : " <<root->info << std::endl;
...
TreeNode* TreeType::PtrToSuccessor_NonRecursive(TreeNode* &tree) {
	...
	if (tempPtr->left == NULL) {
		...
		while (tempPtr->left->left != NULL) {
			tempPtr = tempPtr->left;
					
		}
		
		TreeNode* returnPtr = tempPtr->left;
		tempPtr->left = tempPtr->left->right;
		return returnPtr;
	}
...
void TreeType::Check() {
	...
	std::cout << "origin tree length : " <<this->LengthIs() << std::endl;

	TreeNode* treenode = PtrToSuccessor_NonRecursive(root);
	
	std::cout << "\nroot :" << root->info << std::endl;
	//treenode = PtrToSuccessor_Recursive(root);

	std::cout << "\nmin node  :" << treenode->info;
	std::cout << "\norigin tree length : " << this->LengthIs() << std::endl;
}