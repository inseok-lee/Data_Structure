...
TreeNode* PtrToSuccessor_non_recursive(TreeNode* tree)
{
    TreeNode* tempPtr1 = tree;
    if(tree->left == NULL){
        tree = tree->right;
        return tempPtr1;
    }
...
// 아래의 함수는 tree 전체를 확인하는것이 아닌 해당노드의 child노드들만 검사하기 때문에 binary search tree가 아님에도 true로
// 출력하는 오류가 발생한다.
/*
bool TreeType::IsBST()
{
...
bool Imp_IsBST(TreeNode* tree)
{
    ...
    if(tree != NULL){
        if((tree->left != NULL) && (tree->left->info) > tree->info)
            BST = false;
        else if((tree->right != NULL) && (tree->right->info) < tree->info)
            BST = false;
        BST = Imp_IsBST(tree->left);
        BST = Imp_IsBST(tree->right);
    }
    return BST;
}
*/
